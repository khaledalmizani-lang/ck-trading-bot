...
lines = [f" "---"]
<b>No open trades</b> right now."
<b>Open Trades ({len(_open_trades)}/{config.MAX_OPEN_TRADES})</b>",
...
return " if not trade:
Invalid trade ID. Usage: /close [number]"
...
f" TP: {r['tp_count']} | SL: {r['sl_count']} | Timeout: {r['timeout_
f" {ts}\n══════════════════════")
def _cmd_settings():
...
f" Cooldown: <b>{config.COOLDOWN_MINUTES}min</b>\n"
...
adx = d.get("adx")
if adx and adx >= 25:
<b>Scanning...</b> try again in ~20 seconds."
short = sym.replace("/USDT","").replace("/USDC","")
strong.append((adx, short, d.get("price",0)))
strong.sort(reverse=True)
if not strong:
return " <b>ADX Trend Strength</b>\nNo strong trends right now (ADX < 25 for lines = [" <b>Strong Trend Coins (ADX ≥ 25)</b>", "---"]
...
def _cmd_restart():
import sys
return " send_reply = _reply # captured below in dispatch
...
lines.append(f" <b>{short}</b> -- RSI: <b>{rsi:.1f}</b> Δ{dist:.1f}→{side} | {
lines.append("---")
return "\n".join(lines)
