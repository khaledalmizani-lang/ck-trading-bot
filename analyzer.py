...
def analyze_performance():
    ...
    if enough_data and success_rate < 0.5:
        ...
        if new_buy != old_buy or new_sell != old_sell:
            state["rsi_buy_max"] = new_buy; state["rsi_sell_min"] = new_sell
    ...
    return {"total_evaluated": total_evaluated, "window_size": len(window),
            ...
            "rsi_buy_max": state["rsi_buy_max"], "rsi_sell_min": state["rsi_sell_min"],
            "defensive_mode": state["defensive_mode"], "consecutive_sl": state["consecutive_sl"],
            "adjusted": bool(adjustments), "adjustment_msgs": adjustments}
