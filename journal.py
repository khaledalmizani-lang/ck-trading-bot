...
def log_signal(signal, price, rsi, volume, symbol=None, sl_price=None, tp_price=None):
    ...
    with _lock:
        history = _load()
        # ── Allow up to MAX_OPEN_TRADES simultaneous trades ──
        pending_count = sum(1 for e in history if e["outcome"] == "PENDING")
        if pending_count >= config.MAX_OPEN_TRADES:
            return None
        # ── No duplicate symbol — one trade per symbol at a time ──
        sym = symbol or config.SYMBOL
        if any(e["outcome"] == "PENDING" and e["symbol"] == sym for e in history):
            return None
...
def get_pending_trades():
    ...
    for entry in history:
        ...
        result.append({
            "entry_id": entry["id"],
            "symbol": entry.get("symbol", config.SYMBOL),
            "signal": entry["signal"],
...
def count_summary():
    with _lock:
        history = _load()
    return {
        "success": sum(1 for e in history if e["outcome"] == "SUCCESS"),
